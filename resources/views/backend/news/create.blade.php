...
@section('content') 
@if(isset($news->id))
	{!! Form::model($news, ['method' => 'PATCH','route' => ['news.update', $news->id], 'class'=>'m-form m-form--fit m-form--label-align-right', 'files' => true]) !!}
@else
	{!! Form::open(array('route' => 'news.store','method'=>'POST','class'=>'m-form m-form--fit m-form--label-align-right', 'files' => true)) !!} 
@endif
<div class="row">
        ...
        <div class="m-portlet m-portlet--tab"> 
            <div class="m-portlet__head">
                <div class="m-portlet__head-caption">
                    <div class="m-portlet__head-title">
                        <h3 class="m-portlet__head-text">{{ isset($post->id)?'Edit':'Create New'}} News</h3>
                    </div>
                </div>
                <div class="m-portlet__head-tools">
                    ...
                    <ul class="m-portlet__nav">
                        <li class="m-portlet__nav-item">
                            <a href="{{ route('news.index') }}" class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
                                <span>
            ...
            <div class="m-portlet__body">
                <div class="form-group {{ $errors->has('title') ? 'has-danger':'' }}"> 
                    ...
                    {!! Form::text('title',  $news->title??null, array('placeholder' => 'News Title','class' => 'form-control m-input', 'id'=>'title')) !!} 
                    <div class="form-control-feedback">{{ $errors->first('title') }}</div>
                </div>  

                <div class="form-group {{ $errors->has('description') ? 'has-danger':'' }}"> 
                    {!! Form::label('description', 'Description', array('class' => 'control-label')) !!} 
                    {!! Form::textarea('description', $news->description??null, ['id' => 'editor', 'rows' => 3,  'class' => 'form-control m-input']) !!} 
                ...
                <div class="form-group {{ $errors->has('excerpt') ? 'has-danger' : '' }}"> 
                    ...
                    {!! Form::text('excerpt',  $news->excerpt??'', array('class' => 'form-control m-input', 'id'=>'excerpt')) !!} 
        ...
        <div class="m-portlet m-portlet--tab mb10">
            ...
            <div class="m-portlet__body right-bar"> 
                <div class="form-group">
                    ...
                    {!! Form::select('status', ['Active'=>'Active', 'Inactive'=>'Inactive'], $news->status??old('status'), ['class'=>'form-control c_selectpicker m-input', 'data-rel'=>'chosen']) !!} 
                </div> 
                    ...
                    <div class="form-group m-form__group {{ $errors->has('picture') ? 'has-danger' : '' }}">  
                        <button id="lfm" type="button" data-input="mediaThumbnail" data-preview="mediaHolder" class="img-brows"> 
                            <div class="holder" id="mediaHolder"> 
                                <img src="{{$news->picture?? URL::to('images/default.jpg')}}">
                            </div>
                            <input id="mediaThumbnail" hidden="" value="{{$news->picture?? URL::to('images/default.jpg')}}" type="text" name="picture">
                        </button>  
